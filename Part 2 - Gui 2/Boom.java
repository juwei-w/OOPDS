...
public class Boom extends Game{
    ...
    public static void checkPlayable() {
        if (center.size() != 0) {
            ...
            int numValidCard = 0;
            // System.out.println(currentPlayer.playerCards);

            for (Card card : currentPlayer.playerCards) {
                card.getSuit();
                // System.out.println("Lead = " + leadCard.getSuit() + " " + leadCard.getRank());
                ...
                // System.out.println(numValidCard);
            }
            if (numValidCard == 0) {
    ...
    public static void evaluateWinner() {
        ...
        if (trickNum == 1) {
            while (highestRankCard == leadCard) {
                if (center.get(setHighest).getSuit() == leadCard.getSuit()) { // ----- Same suit
    ...
    public static void loadGame(String fileName) {
        try {
            ...
            while ((dataLine = bufferedReader.readLine()) != null) {
                ...
                // } else if (dataLine.startsWith("Center Map")) {
                // String[] cardStrings = dataLine.split(" = ")[1].split(", ");
                // playedCenter.clear();
                // for (String cardString : cardStrings) {
                //     String suit = cardString.substring(0, 1);
                //     String rank = cardString.substring(1);
                //     Card card = new Card(rank, suit);
                //     playedCenter.put(currentPlayer, card);
                //     }
    ...
    public static void saveGame(String fileName) {
        ...
        try (FileWriter fileWriter = new FileWriter(fileName);
            ...
            // save playedCenter
            bufferedWriter.write("Center Map = ");
            for (Map.Entry<Player, Card> entry : playedCenter.entrySet()) {
                Player player = entry.getKey();
                Card card = entry.getValue();
            ...
            System.out.println("\n" + fileName + " is saved successfully.");
